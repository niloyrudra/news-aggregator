...
test.describe('Navigation', () => {
  ...
  test('should show active filter count in Filter Sidebar', async ({ page }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    await page.locator('button[aria-label="Open filters"]').click();
    
    // Set a sidebar filter (category) to activate filter count
    await page.locator('select').selectOption('Technology');
    
    // Filter count now shows in Filter Sidebar heading "Filters(N)"
    await expect(page.locator('h2:has-text("Filters")')).toContainText(/Filters\(\d+\)/);
  });

  test('should show Clear Filters button in Filter Sidebar when filters active', async ({ page }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    await page.locator('button[aria-label="Open filters"]').click();
